...
class Remixer:
    ...
    def mix(self, path_to_audio, timespan_video, timespan_audio, mute=False, volume = 0.7):
        # FUCK IT!!!
        mute=not mute
        # FUCK IT!!!
...
def main():
    ...
    remixer = Remixer('test/Kerala Beach View in HD Video Kerala Beach Tourism List of Beaches in Kerala Arabian Coast.mp4')

    # 2.Configure
    # use whatever data struct suitable to indicate START POINT and LENGTH of the clip to insert to
    remixer.mix(path_to_audio='music/EndlessUniverse.mp3', timespan_audio=[5, 492], timespan_video=[3, 30], mute=True)
    remixer.mix(path_to_audio='music/vh - 卡农.mp3', timespan_audio=[30, 304], timespan_video=[30, 60], mute=True)
